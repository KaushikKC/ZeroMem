...
function keyForAgent(agentId: string): string {
  ...
  if (
    id.includes("writer") ||
    id.includes("writer-a") ||
    id.includes("writer-v")
  ) {
...
export async function POST(
  ...
  const { agentId = "demo-agent", branch = "main" } = body as {
    agentId?: string;
    branch?: string;
  };
    ...
    switch (params.action) {
      ...
      case "address": {
        ...
        const pubKey = ethers.SigningKey.computePublicKey(
          new ethers.Wallet(privKey).signingKey.publicKey,
          true,
        );
      ...
      case "health": {
        ...
        return ok({
          ...
          kvMode: kv
            ? "in-memory (KV node unreachable — session only)"
            : "on-chain",
          kvNodeDown: kv,
      ...
      case "remember": {
        ...
        const warn = kvFallbackActive(mem)
          ? "KV node unreachable — data stored in-memory this session only. Blobs are on 0G."
          : undefined;
      ...
      case "merge": {
        await mem.merge(body.from, {
          strategy: body.strategy ?? "fast-forward",
        });
      ...
      case "reflect": {
        const commitId = await mem.reflect({
          since: body.since ?? "1h",
          force: body.force,
        });
        return ok({ commitId });
      ...
      case "completePlanTask": {
        const newCommitId = await mem.completePlanTask(
          body.planCommitId,
          body.taskId,
        );
      ...
      case "createChallenge": {
        const challenge = await mem.createAccessChallenge(
      ...
      case "signChallenge": {
        ...
        const recipientKey: string =
          body.recipientPrivKey || process.env.ZG_PRIVATE_KEY!;
        if (!recipientKey || recipientKey.length < 60) {
          return err(
            "recipientPrivKey is required for signChallenge (paste WRITER_PRIVATE_KEY or RESEARCHER_PRIVATE_KEY)",
          );
        }
